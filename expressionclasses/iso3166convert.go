...
// Iso3166Convert implements the Okta Expression Language "Iso3166Convert"
// class, converting between country name, alpha-2, alpha-3, and numeric
// ISO 3166-1 identifiers.
type Iso3166Convert struct{}

func (Iso3166Convert) Call(method string, args ...any) (any, error) {
	switch method {
	case "toAlpha2":
		return iso3166Lookup(args, "toAlpha2", func(c countryCode) string { return c.Alpha2 })
	case "toAlpha3":
		return iso3166Lookup(args, "toAlpha3", func(c countryCode) string { return c.Alpha3 })
	case "toNumeric":
		return iso3166Lookup(args, "toNumeric", func(c countryCode) string { return c.Numeric })
	case "toName":
		return iso3166Lookup(args, "toName", func(c countryCode) string { return c.Name })
	default:
		return nil, fmt.Errorf("Iso3166Convert has no method %q", method)
	}
}

func iso3166Lookup(args []any, method string, field func(countryCode) string) (any, error) {
	if len(args) != 1 {
		return nil, argCountError("Iso3166Convert", method, "1", len(args))
	}
	identifier, ok := args[0].(string)
	if !ok {
		return nil, nil
	}
	country := findCountry(identifier)
	if country == nil {
		return nil, nil
	}
	return field(*country), nil
}
...
func findCountry(identifier string) *countryCode {
